...
`fieldkit_home` replaces the deprecated `data_repo` key. Run `fieldkit config migrate` to update an
older configuration; new configurations must use `fieldkit_home`.
