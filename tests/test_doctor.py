...
from fieldkit.commands.doctor._tools import check_slack_tokens_dead_config, check_tools
...
def test_doctor_bare_all_healthy_exits_zero(tmp_path: Path) -> None:
    with (
        ...
        patch("fieldkit.commands.doctor.cli.check_tools", return_value=[]),
        patch("fieldkit.commands.doctor.cli.check_slack_tokens_dead_config", return_value=None),
    ):
        from fieldkit.commands.doctor._result import DoctorResult
...
# ---------------------------------------------------------------------------
# _tools — CLI-tool-on-PATH + dead Slack config (folded in from setup verify)
# ---------------------------------------------------------------------------


def test_check_tools_reports_found_and_missing() -> None:
    with patch("shutil.which", side_effect=lambda name: "/usr/bin/" + name if name == "claude" else None):
        results = check_tools()
    found = {t.name: t.found for t in results}
    assert found == {"claude": True, "mcpjungle": False, "slackcli": False}


def test_check_slack_tokens_dead_config_both_set_warns() -> None:
    with patch.dict("os.environ", {"SLACK_MCP_XOXC_TOKEN": "xoxc-test", "SLACK_MCP_XOXD_TOKEN": "xoxd-test"}):
        msg = check_slack_tokens_dead_config()
    assert msg is not None
    assert "not actively used" in msg


def test_check_slack_tokens_dead_config_none_set_silent() -> None:
