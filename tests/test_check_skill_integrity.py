...
def test_known_mcpjungle_groups_not_in_opencode_json(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    opencode_config = _REPO_ROOT / "opencode.json"
    if not opencode_config.is_file():
        pytest.skip("operator-specific opencode configuration is excluded from the public tree")
    ocd = opencode_config.read_text(encoding="utf-8")
