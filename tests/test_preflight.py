...
def test_mcp_connect_error() -> None:
    """Returns an error message when the configured MCP gateway is unreachable."""
    import httpx
    ...
    assert result is not None
    assert "MCP gateway not reachable" in result
    assert "configured endpoint" in result


def test_mcp_non_200_response() -> None:
    """Returns an error message when the configured MCP gateway is unhealthy."""
...
def test_mcp_reachable_200() -> None:
    """Returns None when the configured MCP gateway responds with HTTP 200."""
    mock_response = MagicMock()
...
def test_mcp_unexpected_exception() -> None:
    ...
    assert result is not None
    assert "MCP gateway check failed" in result
