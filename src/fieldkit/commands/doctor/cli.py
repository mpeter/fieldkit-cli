"""fieldkit doctor [SERVICE] — consolidated health checks (D6: the setup story).

Bare `fieldkit doctor` runs every service check plus environment/tool checks.
...
def _render_all(results: list[DoctorResult], configuration_states: dict[str, IntegrationConfigurationState]) -> None:
    ...
    for tool in check_tools():
        click.echo(f"  {tool.name}: {'found' if tool.found else 'not found on PATH'}")

    slack_warning = check_slack_tokens_dead_config()
    if slack_warning:
        click.echo(f"  WARN: {slack_warning}")
