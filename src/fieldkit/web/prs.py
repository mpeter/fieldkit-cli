...
def merge_pr(repo: str, number: int, *, gh_runner: Any = run_gh) -> str:
    """Rebase-merge PR *number* and delete its branch.
    ...
    """
    gh_runner(["pr", "merge", str(number), "-R", repo, "--rebase", "--delete-branch"])
    return f"PR #{number} merged (rebase) and branch deleted"
