...
def _check_mcp_reachable() -> str | None:
    """Check that the configured MCP gateway is reachable.
    ...
    Returns:
        None if the configured gateway responds with HTTP 200, or an error message string.
    """
    import httpx
    ...
    try:
        ...
        if response.status_code != 200:
            return f"MCP gateway returned HTTP {response.status_code} at {base}/ — verify its configured endpoint"
        return None
    except httpx.ConnectError:
        return f"MCP gateway not reachable at {base}/ — verify its configured endpoint"
    except Exception as exc:  # noqa: BLE001
        # Catch all other exceptions (timeout, SSL, etc.) — never raise from a check.
        return f"MCP gateway check failed: {exc}"
